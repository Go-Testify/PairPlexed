...
public class DirectorWindow : EditorWindow
{
    ...
    private void DirectorUpdate()
    {
        ...
        float delta = (Time.realtimeSinceStartup - previousTime) / 10;
        if (delta > 0)
        {
        ...
        if (cutscene != null)
        {
            // Update the cutscene if playing
    ...
    private void updateToolbar(Rect toolbarArea)
    {
        ...
        if (cachedCutscenes != null && cachedCutscenes.Length > 0)
        {
            ...
            for (int i = 0; i < cachedCutscenes.Length; i++)
            {
                ...
                if (cutscene == cachedCutscenes[i])
                {
                    popupSelection = i;
                }
            }
            popupSelection = EditorGUILayout.Popup(popupSelection, cutsceneNames, EditorStyles.toolbarPopup);
